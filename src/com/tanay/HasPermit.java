...
public class HasPermit {
	...
	protected int noOfSpecialPermit(Statement statement) {
		ResultSet result = null;
		String query = "SELECT * FROM haspermit WHERE univ_id = '" + this.univ_id + "' AND phone_number = '"
				+ this.phone_number + "' AND special_event='1';";
		int count = 0;
		try {
			result = statement.executeQuery(query);
			if (result.next()) {
	...
	protected void updateDriver(Statement statement, int no_of_permits, boolean inc) {
		...
		String query = "UPDATE driver SET no_of_permits=" + no_of_permits + " WHERE univ_id = '" + this.univ_id
				+ "' AND phone_number = '" + this.phone_number + "';";
		try {
			result = statement.executeQuery(query);
	...
	protected void insert(Statement statement) {
		String query = "INSERT INTO haspermit VALUES ('" + this.univ_id + "','" + this.phone_number + "','"
				+ this.permit_id + "','"
				+ this.special_event + "');";
		try {
