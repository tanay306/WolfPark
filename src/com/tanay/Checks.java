...
public class Checks {
    ...
    protected boolean containsAll(Statement statement, String license_number, String permit_id,
            String citation_number) {
        ResultSet result = null;
        String query;
        if (citation_number == "") {
            query = "Select * from checks where license_number = '" + license_number + "' and permit_id = " + permit_id
                    + " and citation_number is null;";
        } else {
            query = "Select * from checks where license_number = '" + license_number + "' and permit_id = " + permit_id
                    + " and citation_number = " + citation_number + ";";
        }
        try {
            result = statement.executeQuery(query);
            if (result.next()) {
    ...
    protected void insert(Statement statement) {
        ...
        if (this.citation_number.length() == 0) {
            ...
            query = "INSERT INTO checks VALUES ('" + this.license_number + "','" + this.citation_number + "','"
                    + this.permit_id + "');";
        }
