...
public class CitationDAO {
	...
	public void insertCitation(Statement statement, Connection connection) {
		...
		// citation.containsLicenseNumber(statement, license_number)) {
	...
	public void viewAllCitation(Statement statement) {
		...
		try {
			while (result.next()) {
				System.out.println(
						"[(" + result.getString("citation_number") + "), (" + result.getString("citation_date") + "), ("
								+ result.getString("citation_time") + "), (" + result.getString("category") + "), ("
								+ result.getString("payment_status") + "), (" + result.getString("lot_name") + "), ("
	...
	public void viewCitationByFilters(Statement statement) {
		...
		if (citation_number.length() + citation_date.length() + citation_time.length() + category.length()
				+ payment_status.length() + lot_name.length() + zone_id.length() + space_number.length() == 0) {
			...
			try {
				if (result.next()) {
					do {
						System.out.println("[(" + result.getString("citation_number") + "), ("
								+ result.getString("citation_date") + "), (" + result.getString("citation_time")
								+ "), (" + result.getString("category") + "), (" + result.getString("payment_status")
	...
	public void updateCitation(Statement statement) {
		...
		if (citation_number.length() + citation_date.length() + citation_time.length() + category.length()
				+ payment_status.length() + lot_name.length() + zone_id.length() + space_number.length() == 0
				|| citation_number_new.length() + citation_date_new.length() + citation_time_new.length()
						+ category_new.length() + payment_status_new.length() + lot_name_new.length()
						+ zone_id_new.length() + space_number_new.length() == 0) {
