...
public class ChecksDAO {
    ...
    public void viewAllChecks(Statement statement) {
		...
		try {
			while (result.next()) {
				System.out.println("[(" + result.getString("license_number") + "), (" + result.getString("permit_id") + "), (" + result.getString("citation_number") + ")]");
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
    ...
    public void updateChecks(Statement statement) {
        ...
        SQLHelper sqlHelper = new SQLHelper();
        if(license_number.length() + permit_id.length() + citation_number.length() == 0 || license_number_new.length() + permit_id_new.length() + citation_number_new.length() == 0) {
